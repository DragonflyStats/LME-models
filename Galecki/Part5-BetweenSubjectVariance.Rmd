...
Results2 %>% filter( (varW1>0.9)&(varW1<1.1) ) %>% filter((varB1<0.9)|(varB1>1.1))
Results2 %>% filter( (varW2>0.9)&(varW2<1.1) ) %>% filter((varB2<0.9)|(varB2>1.1))
Results2 %>% filter( (varW3>0.9)&(varW3<1.1) ) %>% filter((varB3<0.8)|(varB3>1.2))
Results2 %>% filter( (varW4>0.9)&(varW4<1.1) ) %>% filter((varB4<0.85)|(varB4>1.25))
Results2 %>% filter( (varW5>0.9)&(varW5<1.1) ) %>% filter((varB5<0.75)|(varB5>1.25))
...
Blood4add=matrix(cbind(A1add,A2add,A3add,E1add,E2add,E3add,F1add,F2add,F3add),nrow=samplesize)
Blood=cbind(Blood2,Blood3)
...
blood = groupedData( BP ~ method | subject ,
  data = data.frame( BP = c(Blood), 
    ...
    method = rep(c("A","B","C","D","E","F"), rep(nrow(Blood)*3, 6)),
  	repl = rep(rep(c(1:3), rep(nrow(Blood), 3)), 6) ),
...
```{r}
pairs(Infldf[3:8],pch=18)
```


```{r}

plot(Infldf$methodA,Infldf$methodE,col="red",pch=16,xlab=c("method A"),ylab=c("method E"))
abline(c(0,1))
```
```{r}
plot(Infldf$methodA,Infldf$methodF,col="red",pch=16,xlab=c("method A"),ylab=c("method F"))
abline(c(0,1))
```
```{r}
plot(Infldf$methodE,Infldf$methodF,col="red",pch=16,xlab=c("method E"),ylab=c("method F"))
abline(c(0,1))
...
```{r}
corr.measures(Infldf$methodE,Infldf$methodF)
corr.measures(Infldf$methodA,Infldf$methodF)
corr.measures(Infldf$methodE,Infldf$methodA)
```
```{r}
attach(Infldf)
X<-methodA;  Y<- methodF
