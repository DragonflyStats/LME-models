...
\begin{frame}
  ...
  \item When fitting a large model or if the estimates of the variance
    ...
    criterion.
  \item The optional argument \code{verbose = 2} causes \code{lmer}
...
\begin{frame}[fragile]
  ...
  \begin{center}
<<PenicillinPlot,fig=TRUE,echo=FALSE,height=6.5>>=
...
<<Pastesplot,fig=TRUE,echo=FALSE,height=6.5>>=
...
<<Schoolsplot,fig=TRUE,echo=FALSE,height=6.5>>=
...
\begin{frame}
  ...
  \begin{center}
<<Classpredi,fig=TRUE,echo=FALSE,height=6.5>>=
...
@   
  \begin{center}
<<Classpred2,fig=TRUE,echo=FALSE,height=5.5>>=
...
\begin{frame}[fragile]
  \mode<beamer>{\frametitle{Normal probability plot of random effects for school}}
  \begin{figure}[htb]
    \centering
<<Schoolpred,fig=TRUE,echo=FALSE>>=
print(qqmath(ranef(fm8, post = TRUE),strip=FALSE)$schoolid)
@     
    \mode<article>{\caption{Normal probability plot of random effects for school}}
