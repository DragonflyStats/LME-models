...
\title[GLMM]{Mixed models in R using the lme4 package\\Part 4: Generalized linear mixed models}
...
\begin{frame}
  ...
  \item Random variables
    ...
    \item $\bc U$ the orthogonal random effects, such that $\bc B=\bLt\bc U$
    \end{compactitem}
  \item Parameters
    \begin{compactitem}
    ...
    \item $\bm\theta$ - parameters that determine
      $\mathrm{Var}(\bc B)=\sigma^2\bLt\bLt\trans$ 
    \end{compactitem}
  \item Some matrices
     \begin{compactitem}
     ...
     \item $\bLt$ relative covariance factor, s.t. $\text{Var}(\bc B)=\sigma^2\bLt\bLt\trans$
     \end{compactitem}
   \end{itemize}
 \end{frame}
...
\begin{frame}\frametitle{The conditional distribution,
  ...
  \item For GLMMs, the marginal distribution,
    $\bm{\mathcal{B}}\sim\mathcal{N}\left(\bm
      0,\bm\Sigma_\theta\right)$ is the same as in LMMs except
    that $\sigma^2$ is omitted.  We define
    $\bc U\sim\mathcal{N}(\bm 0,\bm I_q)$ such that
    $\bm{\mathcal{B}}=\bLt\bc U$.
  \item For GLMMs we retain some of the properties of the conditional
    ...
    \begin{displaymath}
      ...
      \bm X\bm\beta+\bm Z\bLt\bm u
    \end{displaymath}
    Specifically
    \begin{itemize}
    \item The conditional distribution, $\bc Y|\bm{\mathcal{U}=\bm u}$, depends
      on $\bm u$ only through the conditional mean,
      $\bm\mu_{\bc Y|\bc U}(\bm u)$.
    \item Elements of $\bc Y$ are \Emph{conditionally
        independent}.  That is, the distribution,
      $\bc Y|\bc U=\bm u$,  is completely
...
\begin{frame}
  ...
  \item When the univariate conditional distributions have constraints
    ...
    linear predictor, $\bm X\bm\beta+\bm X\bLt\bm u$,
    which is unbounded.
...
\begin{frame}
  ...
  \item The linear predictor is
    ...
    X\bm\beta+\bm Z\bLt\bm u$.
    Alternatively, we can write the conditional mean of
    $\bc Y$, given $\bc U$, as
    \begin{displaymath}
      \bm\mu_{\bc Y|\bc U}(\bm u)= \bm g^{-1}\left(\bm X\bm\beta+\bm Z\bLt\bm u\right)
    \end{displaymath}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \item For fixed weights we solve
    ...
    \begin{displaymath}
      ...
      \bm\delta_{\bm u}=\bLt\trans\bm Z\trans\bm M\bm W(\bm y-\bm\mu) - \bm u
    \end{displaymath}
...
\begin{frame}
  ...
  \item At convergence, the sparse Cholesky factor, $\bm L$, used to
    ...
    \begin{displaymath}
      \bm L\bm L\trans =
      \bm P\left(\bLt\trans\bm Z\trans\bm M\bm Z\bLt+\bm I_q\right)\bm P\trans
    \end{displaymath}
...
\begin{frame}
  ...
  \item Notice that this deviance depends on the fixed-effects
    ...
    $\bm\theta$.
  \item In practice we begin by optimizing w.r.t.
    $\bm u$ and $\bm\beta$ simultaneously, evaluating the Laplace
    approximation and optimizing this w.r.t. $\bm\theta$.  Then we
    use a ``pure'' Laplace approximation which is optimized
    w.r.t. both $\bm\beta$ and $\bm\theta$.
  \item The second stage can be suppressed with the optional argument
    \code{nAGQ = 0}.  Another argument \code{verbose = 2} shows the
    two stages explicitly
  \item Another approach is adaptive Gauss-Hermite quadrature
...
\begin{frame}[fragile]\frametitle{Contraception data}
  ...
  \item One of the (binary) responses recorded is whether or not the
...
\begin{frame}[fragile]\frametitle{Showing the optimization stages}
<<cm6stages>>=
...
\begin{frame}\frametitle{Conclusions from the example}
  ...
  \item A generalized linear model is fit with the function
    \code{glmer()} which requires a \code{family} argument.  Typical
    values are \code{binomial} or \code{poisson}
  \item Profiling is not provided for GLMMs at present but will be
    added.
...
\begin{frame}
  \frametitle{A word about overdispersion}
  \begin{itemize}
  \item In many application areas using ``pseudo'' distribution
    families, such as \code{quasibinomial} and \code{quasipoisson}, is
  ...
  \item This amounts to adding an extra parameter, like $\sigma$, the
    common scale parameter in a LMM, to the distribution of the response.
  \item It is possible to form an estimate of such a quantity during
    the IRLS algorithm but it is an artificial construct.  There is no
    probability distribution with such a parameter.
  \item I find it difficult to define maximum likelihood estimates
